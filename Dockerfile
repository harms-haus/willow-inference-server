...
# Ensure PyAV builds with a Cython version it supports (0.29.35+ tighten noexcept
# checks and break PyAV 10.x). Pin to a safe 0.29.x release and disable build
# isolation so it is picked up.
RUN --mount=type=cache,target=/root/.cache \
    pip install "cython>=0.29,<0.29.35" && \
